...
/end_pose #J6 flange pose feedback (geometry_msgs/msg/Pose; legacy interface)
/tcp_pose #gripper-center TCP feedback (geometry_msgs/msg/PoseStamped in base_link)
/joint_states #seven real Piper states for MoveIt/TF (joint1 through joint7)
/joint_states_single #legacy feedback interface (joint0 through joint6)
/joint_ctrl_single #separate command input; never remap from /joint_states
/pos_cmd #end pose control
```

The calibrated TCP is offset `0.1468 m` along the gripper Z axis from J6. It can
be calibrated with the `tcp_offset_x`, `tcp_offset_y`, and `tcp_offset_z`
launch parameters.

`/joint_states` is now published only from real Piper feedback. The six arm
joints map to `joint1` through `joint6`, and total gripper opening maps to the
single independent `joint7` using `gripper_val_mutiple`. URDF `joint8` is a
reverse mimic of `joint7`, so no eighth state is published. Joint commands
remain on `/joint_ctrl_single`.
