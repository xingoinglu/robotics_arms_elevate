...
满足条件后，控制器从真实起点生成S曲线插值，以默认50 Hz、5%速度和至少12秒时长，
通过 `/joint_ctrl_single → JointCtrl` 直接移动到
`[0, 0.4164, -0.5409, 0, 0, 0] rad` 的Ready姿态。该启动过程不进行路径避障规划；
...
| `coarse_lateral_error_min/max` | 配置为 `0.023 / 0.032 m` | 实测面板切平面误差的闭区间；节点内置默认为 `0.025 / 0.035 m` |
...
```text
[0.0, 0.4164, -0.5409, 0.0, 0.0, 0.0] rad
```
